...
namespace AVXXY_NAMESPACE
{
	namespace internals
	{
		struct op_abs : OperationBase
		{
			template<typename S, size_t N>
			static SIMD_Vector<S, N> run(const SIMD_Vector<S, N>& a)
			{
				if constexpr (MaxVectorSize != 0 && sizeof(SIMD_Vector<S, N>) > MaxVectorSize) return { run(a.lo()), run(a.hi()) };
				else return backend(a);
			}

		private:
			template<typename S, size_t N>
			static SIMD_Vector<S, N> backend(const SIMD_Vector<S, N>& a)
			{
