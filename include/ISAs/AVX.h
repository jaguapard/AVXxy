	...
	namespace internals
	{
		namespace ISA
		{
			...
			struct AVX
			{
				...
				template<typename S, size_t N>
					requires (sizeof(S) >= 4)
				static SIMD_Vector<S, N> eval(op_load<S, N>, const void* p, const typename SIMD_Vector<S, N>::MaskT& mask = typename SIMD_Vector<S, N>::MaskT::AllOnes(), const SIMD_Vector<S, N>& src = 0)
				{
					using T = SIMD_Vector<S, N>;
				...
				template<typename S, size_t N>
					requires (sizeof(S) >= 4)
				static void eval(op_store, SIMD_Vector<S, N> vec, void* p, const typename SIMD_Vector<S, N>::MaskT& mask = typename SIMD_Vector<S, N>::MaskT::AllOnes())
				{
					using T = SIMD_Vector<S, N>;
					using F = std::conditional_t<(sizeof(S) > 4), double, float>;
					using I = std::conditional_t<(sizeof(S) > 4), int64_t, int32_t>;
				...
				static typename SIMD_Vector<S, N>::MaskT::UintT eval(op_movemask, const SIMD_Vector<S, N>& a)
				{
					using T = SIMD_Vector<S, N>;
