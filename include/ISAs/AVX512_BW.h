	...
	namespace internals
	{
		namespace ISA
		{
			...
			struct AVX512BW
			{
				...
				template<typename S, size_t N>
				requires (sizeof(SIMD_Vector<S, N>) >= (FS.has(AVX512_VL) ? 0 : 33) && any_small_int<S>)
				static SIMD_BitMask<N> eval(op_vec2mask, const SIMD_Vector<S, N>& v)
				{
					...
					else if constexpr (zmm_sized<T> && any_i16<S>) return _mm512_movepi16_mask(v);
					else if constexpr (zmm_sized<T> && any_i8<S>) return _mm512_movepi8_mask(v);
					else if constexpr (FS.has(AVX512_VL) && ymm_sized<T> && any_i16<S>) return _mm256_movepi16_mask(v);
					else if constexpr (FS.has(AVX512_VL) && ymm_sized<T> && any_i8<S>) return _mm256_movepi8_mask(v);
					else if constexpr (FS.has(AVX512_VL) && xmm_sized<T> && any_i16<S>) return _mm_movepi16_mask(v);
					else if constexpr (FS.has(AVX512_VL) && xmm_sized<T> && any_i8<S>) return _mm_movepi8_mask(v);
